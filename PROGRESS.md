...
**Status: the byte-unit half is done and committed. The model-token half is
built, tested and has never run**, because this environment cannot reach a
tokenizer vocabulary and the workflow that can needs a dispatch I cannot make.

Data: [`benchmarks/results/2026-08-27/`](benchmarks/results/2026-08-27/) —
63 cells, commit `e8a2065`, corpus digest `cd2d48ccf99bddb4`, `git_dirty:
false`. Regenerated by one command in about 100 seconds:
...
51 conversions, 8 failures, 4 successes that produced nothing. Every claim below
is a comparison between backends given the *identical* input; the obvious table
— one row per backend, mean fidelity — was computed and discarded, because
`kreuzberg` scores on eleven fixtures and `pymupdf4llm` on three and those three
are the hardest PDFs in the corpus. A mean over different inputs ranks whichever
backend was asked easier questions.

- **Cost and fidelity are ordered backwards on PDFs.** `pymupdf4llm` is the most
  faithful on all three scorable PDFs (0.848 / 0.972 / 1.000) and the most
  expensive on all three: 234x, 103x and 133x `pypdf`'s wall time, and 280–322
  MiB of resident memory against `pypdf`'s nothing measurable.
- **`twocolumn.pdf` is the argument for fidelity scoring in one row.** Five
  backends, byte counts 4,050 / 4,050 / 4,061 / 4,062 / 4,069 — a 0.47% spread,
  a five-way tie on any cost-only leaderboard — and fidelity from 0.528 to
  0.972. The cost column cannot tell them apart at all.
- **The cheapest output is the one that dropped the table.** On `tables.pdf`,
  `pypdf` emits 481 bytes at 0.333 and `pymupdf4llm` 553 at 0.848. The 15%
  "extra" is the table.
- **Four cells succeed and mean nothing.** `scanned.pdf` has no text layer;
  `pdfplumber`, `kreuzberg`, `markitdown` and `pypdf` all return successfully
  with an empty string, scoring a 100% reduction at 0.000 fidelity.
  `pymupdf4llm` is the only one that calls it a failure. Same shape on
  `jsrendered.html`, where all six HTML backends reduce by 85.1–90.7% at 0.000.
- **The registry's preference order does not match the measurement.** On both
  `.docx` fixtures `libreoffice` is last on fidelity, last on wall time and last
  on memory (0.375 and 0.500; ~1.5 s; 263 MiB), and it fails outright on
  `data.xlsx` and `deck.pptx` in this container. Its value is format coverage,
  not quality, and `docs/BACKENDS.md` now says so.
- **The process boundary costs a consistent ~250 MiB.** `libreoffice` 263 MiB
  both cells, `repomix` 279, `pymupdf4llm` 280–322 — the last being an
  *in-process* backend whose memory `tracemalloc` reports as 0.1 MiB and the
  resident set reports as 300. That divergence is the whole reason both figures
  are published.
- **`gitingest` beats `repomix` on every axis at equal measured fidelity**:
  2,944 bytes against 3,786, 5.6x faster, no subprocess, both 1.000 on the two
  components a packed repository has.

### Defects the harness found in itself

None of these came from a failing test. All five came from reading the output.

1. **`allow_network` was inferred from `bool(extra)`** — a coincidence that
   happened to be right for service backends. Now an explicit flag, recorded in
   the manifest.
2. **`backend_versions` recorded thirteen nulls** under a field documented as
   "the version of every backend that took part". The per-cell versions were
   right the whole time; the map was never filled from them.
3. **The memory column was uninterpretable.** It climbed 50 → 253 → 345 → 375
   MiB in *row order*, because a process-tree peak inherits every import made by
   every earlier cell. Now a baseline is taken immediately before each cell and
   `added RSS` is the difference; the raw peak stays beside it under a paragraph
   saying not to compare peaks between rows.
4. **The repository fixture was silently unscored.** The manifest keys it
   `sample_repo/` and the harness looked up `sample_repo`, so every repo cell
   printed `n/a` — indistinguishable from an honestly unscorable fixture. Caught
   by noticing that `docs/BENCHMARKS.md` recorded a fidelity of 1.000, from the
   CLI, for a cell the matrix called unscorable. `resolve_fixture` had handled
   this since Phase 5 and the harness simply did not use it.
5. **`git_dirty` was always true**, because regenerating into a committed
   results directory dirties the tree. A flag that is always set is worse than
   no flag: a reader takes `false` as a guarantee and stops reading a `true`
   that is always there. Fixing it took two attempts — the first parsed
   porcelain by column, and `_git` strips its output, so the first line arrived
   one character short and the exclusion never matched.

### The proxy block, recorded not routed around

`--tokenizer o200k_base` here does not error. It writes a complete, well-formed,
plausible result set in which **every token column is a dash**, carrying a
warning nobody reads when the exit status says zero:

```
could not load the 'o200k_base' vocabulary: ProxyError:
HTTPSConnectionPool(host='openaipublic.blob.core.windows.net', port=443):
... Tunnel connection failed: 403 Forbidden
```

A CI download failing the same way, with `benchmark.yml`'s `commit` input on,
would have pushed that over the real data. `python -m benchmarks.run` now exits
1 when a unit it was asked for produced no count from any successful cell. Both
directions executed: exit 1 for `o200k_base`, exit 0 for the same command in
`bytes`. A failed *cell* still exits 0 — that is a result.

### Unverified, and it is the largest hole in Phase 10

**No model-token figure for the matrix exists.** `.github/workflows/benchmark.yml`
runs the same matrix in `o200k_base` on a GitHub runner, installs Pandoc and
LibreOffice, checks the corpus against its generator, uploads the artifact
always, and behind an input defaulting to false merges into the committed
byte-unit set and pushes. It has **never run**. Until someone dispatches it, the
token half of `docs/BENCHMARKS.md` does not exist and the page says so in its
first limitation.

This matters more here than it would elsewhere: Phase 7 measured the two units
disagreeing by 24 points on this project's own tabular data and not even ranking
the five serialisation formats in the same order.

### Also done

`docs/BENCHMARKS.md` rewritten around the committed data, with a seven-item
Limitations section linked from the first screen. `benchmarks/README.md`
rewritten. Every `(fixture, backend, number)` triple in `README.md`, `docs/` and
this file checked against `results.json` **by script**: four corrections,
